% DP_TT_CALIBRATION(actio5/MIN_FREQn):
...
% 		end,
    ...
    case 'retry'
		...
		save([OAE_PATH,...
			...
			'BK_amplitude_94dBspl','electrostat'),

		% get "numerical signal level" of BK mic when exposed to 94 dB SPL
		% 1/8": 0.039 (50 dB gain); 1/8": 0.0122 (40 dB gain); 1/2": 0.3135 (20 dB gain)
%         BK_amplitude_94dBspl = 0.039 
        ...
        if electrostat % for electrostatic speakers only
            tone = ((200*tone+212.7).^0.5-sqrt(212.7))/sqrt(200);
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        ...
        tone = sin(linspace(0,TT_ref_freq/MIN_FREQ*2*pi*(1-1/l), l));
		...
		H_wave_HF = H_wave;
		H_wave_HF([1:round(f_crossover/MIN_FREQ), ...
			l-round(f_crossover/MIN_FREQ)+2:l])=0;
		...
		[re im] = pol2cart([0 theta 0 ...
			...
			ones(1,l)./[.1:0.1:l*.1]);
		H_wave_LF = (re +im*i)';
		H_wave_LF(round(f_crossover/MIN_FREQ)+1:l-...
			round(f_crossover/MIN_FREQ)+1)=0;
		wave_LF = real(ifft(H_wave_LF));
		wave_LF = wave_LF./max(abs(wave_LF))/1.5;
		...
		wave_in = data_acquisition([...
            wave_HF, zeros(length(wave),1), wave_LF],...
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if electrostat % for electrostatic speakers only
            wave = ((200*wave+212.7).^0.5-sqrt(212.7))/sqrt(200);
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
        ...
        % TT
        H_TT = data_acquisition([zeros(length(wave_LF),2), wave_LF], 3, 1, ceil(t_avg*MIN_FREQ));
        H_TT = fft(H_TT)./H_wave_LF./H_mic;
        snr_TT = 20*log10(abs(snr_TT./H_mic));
        max_level_TT = snr_TT(round(TT_ref_freq/MIN_FREQ)+1);
	...
	case 'plot'
        up_lim = round(SAMPLE_RATE/2/MIN_FREQ);
		lo_lim = round(TT_ref_freq/MIN_FREQ);
...
% 			x(lo_lim+2:up_lim),...
% 			unwrap(angle(ref_H_rec1(lo_lim+2:up_lim)))'+.025*x(lo_lim+2:up_lim),'b:',...
% 			x(lo_lim+2:up_lim),...
% 			unwrap(angle(ref_H_rec2(lo_lim+2:up_lim)))'+.025*x(lo_lim+2:up_lim),'r:',...
% 			x(3:lo_lim+1),...
% 			unwrap(angle(ref_H_TT(3:lo_lim+1)))','k:',...
% 			x(3:up_lim),...
% 			unwrap(angle(H_mic(3:up_lim)))','g',...
% 			x(lo_lim+2:up_lim),...
% 			unwrap(angle(H_rec1(lo_lim+2:up_lim)))'+ .55*x(lo_lim+2:up_lim),'b',...
% 			x(lo_lim+2:up_lim),...
% 			unwrap(angle(H_rec2(lo_lim+2:up_lim)))'+ .55*x(lo_lim+2:up_lim),'r',...
% 			x(3:lo_lim+1),...
...
% 		zoom on,
		...
		semilogx(x(2:up_lim),20*log10(abs(ref_H_mic(2:up_lim)))....
			...
			/abs(ref_H_rec1(round(1000/MIN_FREQ)+1)))),'b:',...
			x(2:up_lim),20*log10(abs(ref_H_rec2(2:up_lim)...
			/abs(ref_H_rec2(round(1000/MIN_FREQ)+1)))),'r:',...
			x(2:up_lim),20*log10(abs(ref_H_TT(2:up_lim)/...
            abs(ref_H_TT(round(TT_ref_freq/MIN_FREQ)+1)))),'k:',...
			x(2:up_lim),20*log10(abs(H_mic(2:up_lim)))+ ...
			mic_level_correction,'g',...
			x(2:up_lim),20*log10(abs(H_rec1(2:up_lim)/...
			abs(H_rec1(round(1000/MIN_FREQ)+1)))),'b',...
			x(2:up_lim),20*log10(abs(H_rec2(2:up_lim)/...
			abs(H_rec2(round(1000/MIN_FREQ)+1)))),'r',...
			x(2:up_lim),20*log10(abs(H_TT(2:up_lim)/...
            abs(H_TT(round(TT_ref_freq/MIN_FREQ)+1)))),'k-'),
		axis([4 up_lim*MIN_FREQ -80 20]),grid on,
		text(15,17,strcat('green = mic, blue = rec1, red = rec2,',...
			' black=TT, ---- =current, ....=reference)')),
		zoom on,
		...
		semilogx(x(2:up_lim),ref_snr_rec1(2:up_lim),'bx', ...
			x(2:up_lim),ref_snr_rec2(2:up_lim),'rx',...
			x(2:up_lim),ref_snr_TT(2:up_lim),'kx',...
			x(2:up_lim),snr_rec1(2:up_lim),'bo', ...
			x(2:up_lim),snr_rec2(2:up_lim),'ro', ...
			x(2:up_lim),snr_TT(2:up_lim),'ko'),
		axis([4 up_lim*MIN_FREQ -50 120]), grid,
		text(12,115,['                blue = rec1, ',...
			'  red = rec2,  black = TT,  (o = current,  x= reference)']),
