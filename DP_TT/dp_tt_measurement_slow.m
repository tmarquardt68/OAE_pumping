...
if probe_check == 0, % CURRENT_EAR is filled with data of last calibration
    ...
    if ~strcmp('Yes',questdlg(strvcat(strvcat(...
            ...
            num2str((2*F1(1)-F2(1))*MIN_FREQ) ' ' num2str((F2(2)-F1(1))*MIN_FREQ) ' ' ...
...
Hdb_TT = 20*log10(abs(CURRENT_EAR(F_TT+1,1)'));
if isinf(F_TT(1))
    Hdb_TT = inf;
else
    Hdb_TT = 20*log10(abs(CURRENT_EAR(F_TT+1,1)'));
    if max(L_TT - Hdb_TT) > CURRENT_EAR(1,5),
...
if ~exist([OAE_PATH,'\Subjects\',patient]),
...
timeStamps=NaN*ones(1,n);
a = 1;
while a <= n
	% phase correction
	wave = zeros(3,l);
	[re im] = pol2cart(-pi/2-angle(CURRENT_EAR(F1(a)+1,1)),l/2);
	wave(1,F1(a)+1) = re + im*i;
	wave(1,l - F1(a)+1) = re - im*i;
	[re im] = pol2cart(-pi/2-angle(CURRENT_EAR(F2(a)+1,2)),l/2);
	wave(2,F2(a)+1) = re + im*i;
	wave(2,l - F2(a)+1) = re - im*i;
    ...
    if L_TT(a) > 0 
        ...
        wave(3,F_TT(a)+1) = re + im*i;
        wave(3,l - F_TT(a)+1) = re - im*i;
        wave(3,:) = real(ifft(wave(3,:)));
        ogainT = 10^( (L_TT(a)-Hdb_TT(a)-CURRENT_EAR(1,5))/20);
        wave_out(1,:)=wave(1,:)*ogain1 + wave(3,:)*ogainT;
    else
        wave_out(1,:)=wave(1,:)*ogain1;
    end
    % Volume setting via wave amplitude!

% 	% acoustic signal indicating begin measurement
% 	sound_io(SAMPLE_RATE, sum(sine(SAMPLE_RATE,[501,1000]))/10, 1),
% 	pause(1)
	% the measurement
	[wave_in, n_used]  = data_acquisition(wave_out', 2, 2,...
		ceil(t_avg*MIN_FREQ));
	H_avg_acoust = fft(wave_in(:,1))./h_mic;
	H_avg_CM = fft(wave_in(:,2));
