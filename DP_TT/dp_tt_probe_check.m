...
switch action
    ...
    case 'retry'
		...
		[re im] = pol2cart([0 theta 0 ...
			...
			ones(1,l)); %./linspace(1,480000,SAMPLE_RATE/MIN_FREQ));
% 		[re im] = pol2cart([0 theta 0 ...
% 			-theta(l/2-1:-1:1)],...
% 			ones(1,l));
		H_wave = (re +im*i)';
...
% 			round(10000/MIN_FREQ)+1)=0;
		...
		wave_LF = wave_LF./max(abs(wave_LF))/10;
...
%		CURRENT_EAR(:,3)= H_TT;
		...
		semilogx(...
			x(TT_ref_freq/MIN_FREQ+1:l/2),...
			20*log10(abs(CURRENT_EAR(TT_ref_freq/MIN_FREQ+1:l/2,1)./...
			H_rec1(TT_ref_freq/MIN_FREQ+1:l/2))),'b',...
            x(TT_ref_freq/MIN_FREQ+1:l/2),...
            20*log10(abs(CURRENT_EAR(TT_ref_freq/MIN_FREQ+1:l/2,2)./...
            H_rec2(TT_ref_freq/MIN_FREQ+1:l/2))),'r',...
			x(TT_ref_freq/MIN_FREQ+1:f_crossover/MIN_FREQ),...
            20*log10(abs(CURRENT_EAR(TT_ref_freq/MIN_FREQ+1:f_crossover/MIN_FREQ,3)./...
            H_TT(TT_ref_freq/MIN_FREQ+1:f_crossover/MIN_FREQ))),'k',...
			x,zeros(1,length(x)),'g:'), 
        axis([TT_ref_freq 24000 -20 20]),grid on
...
%         set(h_axes(2),'Tag','axes_phase'),
		...
		semilogx(...
			x(TT_ref_freq/MIN_FREQ+1:l/2),...
			20*log10(abs(CURRENT_EAR(TT_ref_freq/MIN_FREQ+1:l/2,1)./...
			H_rec1(1000/MIN_FREQ+1)))+CURRENT_EAR(1,5),'b',...
			x(TT_ref_freq/MIN_FREQ+1:l/2),...
            20*log10(abs(CURRENT_EAR(TT_ref_freq/MIN_FREQ+1:l/2,2)./...
            H_rec2(1000/MIN_FREQ+1)))+CURRENT_EAR(2,5),'r', ...
            x(TT_ref_freq/MIN_FREQ+1:f_crossover/MIN_FREQ),...
            20*log10(abs(CURRENT_EAR(TT_ref_freq/MIN_FREQ+1:f_crossover/MIN_FREQ,3)./...
            H_TT(TT_ref_freq/MIN_FREQ+1)))+CURRENT_EAR(3,5),'k'), 
        grid on, zoom on,
        set(h_axes(2),'Tag','axes_phase'),
        linkaxes(h_axes,'x')        
        axis([TT_ref_freq 24000 60 140]),grid on
        ...
        CURRENT_EAR(:,3)=CURRENT_EAR(:,3)./...
            abs(H_TT(round(TT_ref_freq/MIN_FREQ)+1));
